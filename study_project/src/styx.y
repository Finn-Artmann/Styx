...
global_declaration: GLOBAL TYPE ID SEMICOLON { $$ = new_astnode(GLOBAL_DECLARATION); $$->val.str = $3; $$->data_type = $2; }
		| GLOBAL TYPE ID ASSIGN expression SEMICOLON { $$ = new_astnode(GLOBAL_DECLARATION_ASSIGN); $$->val.str = $3; $$->data_type = $2; $$->child[0] = $5; }


declaration: TYPE ID SEMICOLON { $$ = new_astnode(DECLARATION); $$->val.str = $2; $$->data_type = $1; }
		| TYPE ID ASSIGN expression SEMICOLON { $$ = new_astnode(DECLARATION_ASSIGN); $$->val.str = $2; $$->data_type = $1; $$->child[0] = $4; }
...
for_statement: FOR ROUND_OPEN declaration expression SEMICOLON assignment ROUND_CLOSE CURLY_OPEN body CURLY_CLOSE { $$ = new_astnode(FOR); $$->child[0] = $3; $$->child[1] = $4; $$->child[2] = $6; $$->child[3] = $9; }
			| FOR ROUND_OPEN factor ROUND_CLOSE CURLY_OPEN body CURLY_CLOSE { $$ = new_astnode(FOR_NUM); $$->child[0] = $3; $$->child[1] = $6; }

repeat_statement: REPEAT CURLY_OPEN body CURLY_CLOSE UNTIL ROUND_OPEN expression ROUND_CLOSE SEMICOLON { $$ = new_astnode(REPEAT); $$->child[0] = $3; $$->child[1] = $7; }
...
function_call: ID ROUND_OPEN arguments ROUND_CLOSE { $$ = new_astnode(FUNCTION_CALL); $$->val.str = $1; $$->data_type = AST_ID_T; $$->child[0] = $3; }
			| SYSTEM ROUND_OPEN STR ROUND_CLOSE { $$ = new_astnode(SYSTEM_CALL); $$->data_type = AST_INT_T; $$->val.str = $3; }
...
term: factor { $$ = new_astnode(TERM_FACTOR); $$->child[0] = $1; $$->data_type = $$->child[0]->data_type; }
	...
	| NOT factor { $$ = new_astnode(TERM_NOT_FACTOR); $$->child[0] = $2; }

factor: ID { $$ = new_astnode(FACTOR_ID); $$->val.str = $1; $$->data_type = AST_ID_T; }
    | NUM { $$ = new_astnode(FACTOR_NUM); $$->val.num = $1; $$->data_type = AST_INT_T; $$->is_const = 1;}
	| REAL { $$ = new_astnode(FACTOR_REAL); $$->val.real = $1; $$->data_type = AST_DOUBLE_T; $$->is_const = 1; }
	| STR { $$ = new_astnode(FACTOR_STRING); $$->val.str = $1; $$->data_type = AST_STR_T; $$->is_const = 1; }
	| CHR { $$ = new_astnode(FACTOR_CHAR); $$->val.chr = $1; $$->data_type = AST_CHAR_T; $$->is_const = 1; }
